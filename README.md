...
- **Pyodide 버전**: `worker.js` 상단의 `PYODIDE_VERSION` 값으로 CDN에서 받아옵니다.
  로딩이 안 되면 이 값을 최신 안정 버전으로 바꿔 보세요.
- **"브라우저 격리가 꺼져 있어 실행할 수 없어요"** 가 보이면, 교차 출처 격리가 안 켜진 것입니다.
  GitHub Pages(https)인지, 첫 새로고침이 일어났는지 확인하세요. 사파리보다 **안드로이드 크롬**이
  가장 잘 동작합니다. 회사/학교 네트워크나 일부 확장프로그램이 서비스 워커를 막기도 합니다.
- **첫 로딩이 느림**: 파이썬 런타임(WebAssembly)을 받는 과정이라 처음 한 번만 그렇습니다.
- 원본의 타이핑 연출(`slow`)·대기(`time.sleep`)는 제거했습니다(웹 화면 전환이 대신).
